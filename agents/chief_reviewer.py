...
def chief_reviewer_node(state: dict) -> dict:
    ...
    capital_report = state.get("capital_report")
    ...
    has_all_reports = all([
        capital_report, fund_report, industry_report,
        risk_report, tech_report, valuation_report
    ])
    
    if has_all_reports:
        reviewer = ChiefReviewerAgent(DEFAULT_MODEL)
        final_report = reviewer.synthesize_reports(
            stock_code,
            capital_report,
            fund_report,
            industry_report,
